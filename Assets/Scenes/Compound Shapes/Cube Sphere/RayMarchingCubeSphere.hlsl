// RAYMARCHING_CUBESPHERE is defined, otherwise Shader Graph cannot compile this node.
// This definition should be unique to this function. So, it is recommended to define it similar to the function name.
#ifndef RAYMARCHING_CUBESPHERE

    #define RAYMARCHING_CUBESPHERE
    ...
    float Shape(float3 position) {
        float cube = Cube(position, _shapeOptions.cubeSize); // calculate the distance to the cube surface
        float sphere = Sphere(position, _shapeOptions.sphereRadius); // calculate the distance to the sphere surface
        float shape = SminQuadratic(cube, sphere, _shapeOptions.smoothness); // blend the two shapes using a smooth minimum function
        return shape;
    }
    ...
    void RayMarchingCubeSphere_float(float3 cubeSize, float sphereRadius, float smoothness, float3 rayStartPoint, float3 rayDirection, float minDistance, float maxDistance, int maxIterations, float3 lightDirection, half4 color, float brightness, out half4 Out) {
