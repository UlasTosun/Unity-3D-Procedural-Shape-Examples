// RAYMARCHING_CAPSULESPHERE is defined, otherwise Shader Graph cannot compile this node.
// This definition should be unique to this function. So, it is recommended to define it similar to the function name.
#ifndef RAYMARCHING_CAPSULESPHERE

    #define RAYMARCHING_CAPSULESPHERE
    ...
    float Shape(float3 position) {
        ...
        float shape = SminQuadratic(capsule, sphere, _shapeOptions.smoothness); // blend the two shapes using a smooth minimum function
        return shape;
    }
    ...
    void RayMarchingCapsuleSphere_float(float capsuleHeight, float capsuleRadius, float sphereRadius, float smoothness, float3 rayStartPoint, float3 rayDirection, float minDistance, float maxDistance, int maxIterations, float3 lightDirection, half4 color, float brightness, out half4 Out) {
