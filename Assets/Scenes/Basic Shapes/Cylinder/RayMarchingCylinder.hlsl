// RAYMARCHING_CYLINDER is defined, otherwise Shader Graph cannot compile this node.
// This definition should be unique to this function. So, it is recommended to define it similar to the function name.
#ifndef RAYMARCHING_CYLINDER

    #define RAYMARCHING_CYLINDER
    ...
    float Shape(float3 position) {
        float cylinder = Cylinder(position, _shapeOptions.height, _shapeOptions.radius); // calculate the distance to the cylinder
        return cylinder;
    }
    ...
    void RayMarchingCylinder_float(float height, float radius, float3 rayStartPoint, float3 rayDirection, float minDistance, float maxDistance, int maxIterations, float3 lightDirection, half4 color, float brightness, out half4 Out) {
        _shapeOptions.height = height; // set the height of the object
        _shapeOptions.radius = radius; // set the radius of the object
