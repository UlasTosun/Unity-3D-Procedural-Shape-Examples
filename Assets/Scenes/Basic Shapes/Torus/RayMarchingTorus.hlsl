// RAYMARCHING_TORUS is defined, otherwise Shader Graph cannot compile this node.
// This definition should be unique to this function. So, it is recommended to define it similar to the function name.
#ifndef RAYMARCHING_TORUS

    #define RAYMARCHING_TORUS
    ...
    struct ShapeOptions {
        ...
        float tubeRadius; // The radius of the tube of the torus.
    };
    ...
    float Shape(float3 position) {
        float torus = Torus(position, _shapeOptions.radius, _shapeOptions.tubeRadius); // calculate the distance to the torus
        return torus;
    }
    ...
    void RayMarchingTorus_float(float radius, float tubeRadius, float3 rayStartPoint, float3 rayDirection, float minDistance, float maxDistance, int maxIterations, float3 lightDirection, half4 color, float brightness, out half4 Out) {
        _shapeOptions.radius = radius; // set the radius of the torus
        _shapeOptions.tubeRadius = tubeRadius; // set the radius of the tube of the torus
